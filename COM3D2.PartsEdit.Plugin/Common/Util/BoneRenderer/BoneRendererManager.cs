...
///  Updateにより位置を適宜更新する.
///
internal class BoneRendererManager {//: MonoBehaviour {
    #region Fields
    private const string NAME_PREFIX = "___";
    ...
    public void Setup(GameObject go) {
        ...
        if (parentBone != null) {
            ...
            foreach (Transform child in parentBone) {
                if (!endVisible && child.childCount == 0) continue;
    ...
    private void SetupBone(Transform bone) {
        ...
        foreach (Transform child in bone) {
            if (!endVisible && child.childCount == 0) continue;
            if (child.name.StartsWith(NAME_PREFIX)) continue;
    ...
    public void UpdatePosition(Transform bone, bool isRoot = false, bool isRecursive = false) {
        ...
        if (bone.childCount == 0) {
            if (endVisible) {
                ...
                boneLine.SetPosition(1, new Vector3(-minLength, 0f, 0f));
                return;
            } else {
                EmptyBone(boneLine);
                return;
        ...
        if (bone.childCount == 1) {
            ...
            if (child0.name.StartsWith(NAME_PREFIX)) {
                EmptyBone(boneLine);
                return;
            }
            pos = child0.position;
            ...
            if (bone.childCount == 2) {
                ...
                if (child0.name.EndsWith(NAME_SCL) || child0.name.StartsWith(NAME_PREFIX)) {
                    pos = child1.position;
                } else if (child1.name.EndsWith(NAME_SCL) || child1.name.StartsWith(NAME_PREFIX)) {
                    pos = child0.position;
                }
            }
            if (!pos.HasValue) {
    ...
    public void SetParent(Transform fParent) {
        cacheParent = fParent;
    }
    ...
    private LineRenderer CreateComponent(Material material, float startWidth, float endWidth) {
        ...
        if (cacheParent) {
            obj.transform.parent = cacheParent;
        }
        _cache.Add(obj);
